# Grocery List — 13 guys, Sep 17–21

**Owner: Chris Psilos.** Store: **Smith's (Kroger)** or **Albertsons**, Jackson. Buy **Thursday on arrival** (or pre-stock — see [delivery options](./dining.md#grocery-delivery-options)).

---

## Read this first — the list is smaller than you'd expect

13 guys × 4 nights sounds like a mountain of food. It isn't, because **most meals on this trip are out.** This list feeds exactly six slots:

| Slot | On this list? | Why |
|---|---|---|
| Thu dinner | ❌ | Bar crawl — Snake River Brewing has a full menu |
| Fri breakfast | ❌ | **Cowboy Coffee drive-thru** (1007 S US-89, opens 6 AM), on the way to the 7:45 AM check-in |
| **Fri lunch** | ✅ | ⚠️ **Newly on the list.** JHWW feeds their *scenic float*, not our whitewater trip — so Friday's lunch is sandwiches, packed Thursday |
| **Fri dinner** | ✅ | House. **Menu defaults to the fajita bar** (Module A) unless the Sep 10 call says otherwise — see the two modules below |
| **Sat breakfast** | ⚠️ partial | Burrito stash, bought from a **vendor**, not the grocery store |
| **Sat lunch** | ✅ | Cooler lunch, eaten at West Thumb |
| **Sat dinner** | ✅ | **Steak night** — the ribeyes |
| **Sun breakfast** | ⚠️ partial | Burrito stash again |
| **Sun late-morning** | ✅ | Saints game spread, 11 AM kickoff |
| Sun dinner | ❌ | Dornan's in Moose *(unless steak night moves here)* |
| **Mon breakfast** | ✅ | Leftovers + coffee, out the door by 9:45 |

**So the real shopping is:** one house dinner, one cooler lunch, one steak night, one game-day spread, coffee for four mornings, snacks for two park days, and the drinks for three house nights.

---

## ⚠️ Two open decisions change this list

1. **Friday dinner menu** — fajita bar vs. one-pot. This is the *only* part of the list that actually branches. Pick a module below. ✅ **Default: Module A, the fajita bar** — nobody has to decide anything for that to happen, and it's the branch that also feeds Saturday's cooler lunch.
2. **Steak night: Saturday or Sunday?** — **does not change the list at all.** Same ribeyes, same sides, different night. Don't let this one hold up the shopping.

A third one changes a single line item: **the grill check** (gas or charcoal?). If it's charcoal, add charcoal + a chimney starter. Drewes asks RMR on the **9/7** call — and you eyeball the grill yourself Thursday at check-in, which is the check that actually protects the night. See [`dining.md`](./dining.md#grill-check--before-committing-to-ribeyes).

> ⚠️ **Friday lunch is new to this list.** It can't come from Friday's leftovers — Friday dinner happens *after* it. Buy for **two** cooler lunches, not one: Friday at the boathouse and Saturday at West Thumb.

**Also on this trip, and ✅ no longer your problem:** bear spray. It came off this run entirely — the crew now **rents 2 canisters at the Bear Aware kiosk in JAC baggage claim** on arrival (~$56 total, returned Monday). Nothing to buy, no sporting-goods stop. *(If the kiosk is out, **Smith's and Albertsons both stock it** — one line on this cart, not a separate trip.)*

---

## 🥩 Step 1: The butcher call — do this before anything else

> This is the one item you cannot leave to shelf stock or a delivery app. **13 × 24 oz bone-in ribeyes is ~19.5 lbs of steak** and no grocery meat case in a resort town has that sitting out on a Thursday afternoon.

- [ ] **Call the Albertsons butcher counter ahead** — ask them to cut and hold **13 bone-in ribeyes, 24 oz each (~1.5 lb)**. They're usually the one carrying **Prime**; **Smith's** and **Jackson Whole Grocer** are the backups
- [ ] Give them the **pickup day (Thursday)** and a name
- [ ] Ask what they've got if they can't do 13 — 20 oz × 13 is a fine fallback; splitting across Albertsons *and* Smith's is the backup
- [ ] **Pick these up in person.** Delivery apps pull from shelf stock — this is the one line that has to be a human at a counter

**Timing note:** buying Thursday works whether steak night lands Saturday or Sunday. Refrigerate — **do not freeze bone-in ribeyes** for a two-day hold.

---

## Step 2: The master list

Everything below is **shared regardless of the Friday menu**. Then add one module.

### 🥩 Meat & Deli

- [ ] **13 bone-in ribeyes, 24 oz** — butcher counter, pre-ordered (see above)
- [ ] Deli turkey + ham, **6 lb total** — **two** cooler lunches: Friday at the boathouse + Saturday at West Thumb
- [ ] Sliced cheese, **1.5 lb** — same
- [ ] Chicken wings, **10 lb** — Sunday game spread
- [ ] Andouille or smoked sausage, **3 lb** — Sunday game spread

### 🥬 Produce

- [ ] Russet or red potatoes, **10 lb** — steak night side
- [ ] Corn on the cob, **13 ears** — steak night side
- [ ] Bagged salad kits, **3** — the green thing nobody plans and everybody eats
- [ ] Yellow onions, **3 lb**
- [ ] Lemons/limes, **6**
- [ ] Apples or oranges, **10** — cooler lunch + car snacks
- [ ] Veggie tray + ranch, **1 large** — Sunday spread

### 🍞 Bread & Bakery

- [ ] Sandwich rolls or sliced bread, **3 loaves / 2 dozen rolls** — Saturday cooler lunch
- [ ] Bagels or English muffins, **2 dozen** — Monday breakfast + backup
- [ ] Hamburger buns, **1 pack** — game-day sausage

### ☕ Breakfast & Coffee

> **Coffee is the single most under-bought item on a trip like this.** 13 guys, four mornings, two of them pre-dawn. Buy more than feels right.

- [ ] **Ground coffee, 3 lb** — and check whether the house is drip, Keurig, or both before you commit to a format
- [ ] Coffee filters — **buy a pack even if you think the house has them**
- [ ] Creamer (2 large) + sugar
- [ ] Eggs, **2 dozen** — backup only; Sat/Sun mornings are the burrito stash
- [ ] Bacon, **2 lb** — Monday / lazy morning
- [ ] Butter, **2 lb** — steak night needs it too
- [ ] Cream cheese, jam, peanut butter

### 🧂 Pantry & Seasoning

> Airbnbs never stock enough salt for 13 steaks. Assume the kitchen has nothing.

- [ ] **Kosher salt** (big box), **coarse black pepper**, **garlic powder** — the ribeye rub
- [ ] Olive oil / cooking oil
- [ ] Mayo, mustard, ketchup — cooler lunch
- [ ] Hot sauce — **2 bottles**, the burritos want it
- [ ] BBQ sauce + wing sauce — Sunday
- [ ] Sour cream, **2 large**
- [ ] Chips, **4 big bags** + salsa/queso/dip, **4** — Sunday spread
- [ ] Trail mix, jerky, granola bars — **park-day car snacks, Sat + Sun**

### 🧊 Drinks

- [ ] **Beer — 6 cases.** Thursday is at bars and Friday everyone's wrecked from the river, so this is really Fri/Sat/Sun at the house
- [ ] **Whiskey — 2 handles** + mixers (Coke, ginger ale, soda water)
- [ ] Hard seltzer, **2 cases**
- [ ] Bottled water, **3 cases** — altitude; you'll drink more than at home
- [ ] Gatorade/electrolytes, **1 case** — 6,200 ft and a bar night. This is the most useful $12 on the list
- [ ] Ice, **6 bags** — and plan to re-buy; a cooler in a September car eats through it

### 🧻 Paper & Supplies

- [ ] Solo cups (100+), paper plates (150+), napkins, paper towels (6 rolls)
- [ ] Trash bags, **2 boxes** — 13 guys generate more than you think, and you're hauling it out Monday
- [ ] **Heavy-duty foil** — reheating burritos, wrapping steaks to rest
- [ ] Dish soap + dishwasher pods
- [ ] Gallon ziplocs — cooler lunch, leftovers
- [ ] Sunscreen / lip balm if anyone forgot *(they did)*
- [ ] ⚠️ **Charcoal (2 bags) + chimney starter** — **only if the grill turns out to be charcoal.** Skip if it's gas and the tank's full; if the tank's low, a **~$25 propane exchange** is at the same Smith's/Albertsons

---

## Step 3: Pick ONE Friday dinner module

### 🌮 Module A — Fajita / Taco Bar

*Assembly line, everyone builds their own, scales past 13 without a real cook.*

- [ ] Chicken thighs, boneless, **10 lb**
- [ ] Skirt or flank steak, **5 lb**
- [ ] Flour tortillas, **60**
- [ ] Bell peppers, **10** + yellow onions, **5 lb** *(on top of the 3 lb above)*
- [ ] Shredded cheese, **2 lb**
- [ ] Salsa, **3 jars**
- [ ] Avocados, **12** — buy a couple ripe, the rest firm
- [ ] Rice, **3 lb** + canned black beans, **6**
- [ ] Fajita seasoning, **4 packets**
- [ ] Cilantro, extra limes

✅ **Bonus: this module feeds Saturday's cooler lunch.** Leftover fajita meat + tortillas = wraps for Yellowstone — so cut the deli line back to **3 lb** (Friday's lunch still needs it) and save ~$18. ⚠️ **It cannot feed Friday's lunch**, which happens before this dinner is cooked.

### 🍲 Module B — One-Pot (Jambalaya)

*Cheaper, less prep, one person cooks and walks away while everyone else showers off the river.*

- [ ] Andouille sausage, **4 lb** *(on top of the 3 lb for Sunday)*
- [ ] Chicken thighs, boneless, **5 lb**
- [ ] Long-grain rice, **4 lb**
- [ ] The trinity — onion, bell pepper, celery *(3 each)*
- [ ] Chicken stock, **4 quarts**
- [ ] Diced tomatoes, **2 large cans**
- [ ] Creole seasoning, bay leaves, thyme
- [ ] French bread, **3 loaves**

⚠️ **If you pick B, keep the full 6 lb of deli meat + rolls above** — jambalaya doesn't travel into a cooler the way fajita leftovers do, so both Friday's and Saturday's lunches come off the deli line.

---

## What this runs

| Section | Fajita branch | One-pot branch |
|---|---|---|
| Ribeyes (13 × 24 oz) | ~$330 | ~$330 |
| Friday dinner module | ~$195 | ~$100 |
| Meat, deli & produce | ~$150 | ~$185 |
| Breakfast, coffee & pantry | ~$180 | ~$180 |
| Sunday game spread | ~$115 | ~$115 |
| Drinks | ~$280 | ~$280 |
| Paper, ice & supplies | ~$85 | ~$85 |
| **Total** | **≈ $1,335** | **≈ $1,275** |
| **Per person (13)** | **≈ $103** | **≈ $98** |

**Cross-check against [`/budget`](../budget/budget.md):** the budget carries *Food & Drink $150–250* plus *Ribeyes $50–80* per person — call it **$200–330**. This list lands at **~$100/person**, with the restaurant meals (Thursday bars, Friday rafting lunch, Sunday Dornan's) making up the rest. **The budget holds.**

Not on this list, but real money:
- **Breakfast burritos, ~18–20 from a vendor — ~$200 total (~$15/person).** Ordered from Cowboy Coffee / Pearl Street / Picnic / D.O.G. / Hand Fire and picked up **Friday after rafting**. See [`dining.md`](./dining.md#breakfast-burritos-for-the-early-starts--house-stash)

---

## Splitting the run: what can be delivered vs. what can't

Psilos still owns the [in-person vs. pre-stock call](./dining.md#grocery-delivery-options). Either way, the split is the same:

| Must be in person | Safe to pre-stock or Instacart |
|---|---|
| **The 13 ribeyes** (butcher counter) | Drinks, ice, paper goods |
| Produce you care about (avocados) | Pantry, seasoning, canned goods |
| — | Breakfast, coffee, snacks |

**The smart play:** pre-stock the bulk and drinks so the house is loaded when 13 guys walk in mid-afternoon Thursday, pick up the steaks in person, and keep Instacart as the Saturday restock button. 🆕 **Thursday does now have a real 3:00–4:00 PM block for shopping** — the Bridger Gondola closed for the season on Sept 13, so its slot in the afternoon plan became the errand hour. Pre-stocking turns that hour into an early check-in instead.

---

## Thursday, at the store — the short version

1. Steaks are **pre-ordered and waiting** at the butcher counter
2. One guy on drinks + ice, one on the master list, one on the Friday module — **three carts, not thirteen guys wandering the store**
3. **Ice goes in last**, and straight into the cooler
4. Bear spray is **off this list** — it's a rental at the JAC baggage-claim kiosk now. *(Backup: Smith's and Albertsons do carry it, so it's one line here if the kiosk falls through — it was never a separate sporting-goods trip.)*
